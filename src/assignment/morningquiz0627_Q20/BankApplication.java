...
class BankApplication extends Account {
  ...
  Account[] accounts = new Account[100]; // 관리 계좌는 총 100개
  ...
  void openAccount() { // 계좌생성
    ...
    if (currentAccCount >= 100) { // 관리계좌 수를 넘기면 계좌생성불가
  ...
  void depositResult() { // 예금
    ...
    System.out.print("계좌번호: ");
    accNum = sc.nextLine();
    for (int i = 0; i < currentAccCount; i++) {
      if (accNum.equals(accounts[i].accNum)) { // 등록된 계좌번호인지 확인
        System.out.print("예금액: ");
        accounts[i].balance += sc.nextInt(); // 해당 계좌에 입금
        sc.nextLine();
        break;
      } else if (i == currentAccCount-1){ // 등록된 계좌가 아닐경우
  ...
  void withdrawalResult() {
    ...
    for (int i = 0; i < currentAccCount; i++) {
      if (accNum.equals(accounts[i].accNum)) { // 등록된 계좌번호인지 확인
        ...
        sc.nextLine();
        if (withdrawalAmount > balance) { // 잔액보다 출금액이 크면
